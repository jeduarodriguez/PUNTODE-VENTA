...
interface CustomersProps {
  ...
  onProcessPayroll?: (workerId: string, amountToPay: number, amountToDeductDebt: number, method: 'Cash' | 'Card' | 'PagoMovil' | 'Transfer', rate?: number) => void;
}

const Customers: React.FC<CustomersProps> = ({ customers, workers, sales, exchangeRate, businessDebts, rateHistory, onAdd, onUpdate, onDelete, onDebtPayment, onAddWorker, onUpdateWorker, onDeleteWorker, onWorkerDebtPayment, onAddBusinessDebt, onPayBusinessDebt, onUpdateBusinessDebt, onDeleteBusinessDebt, onProcessPayroll }) => {
  ...
  const handleProcessPayroll = (method: 'Cash' | 'Transfer' | 'PagoMovil') => {
    selectedWorkers.forEach(workerId => {
      ...
      if (worker && onProcessPayroll) {
        const debt = worker.balance || 0;
        const amountDeducted = Math.min(worker.salary, debt);
        const amountHandedOver = Math.max(0, worker.salary - amountDeducted);
        if (amountHandedOver > 0 || amountDeducted > 0) {
          onProcessPayroll(workerId, amountHandedOver, amountDeducted, method, todayRate);
        }
      }
    });
    ...
    <div className="space-y-4 pb-24">
      ...
      <div className="flex w-full md:w-auto gap-2">
        <div className="relative flex-1 md:w-64">
          ...
          <input
            type="text"
            placeholder={activeTab === 'customers' ? "Buscar cliente..." : activeTab === 'workers' ? "Buscar trabajador..." : "Buscar deuda..."}
            className="w-full pl-10 pr-4 py-3 bg-white border-2 border-gray-100 rounded-2xl text-black font-bold text-sm focus:border-indigo-500 outline-none transition-colors"
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="space-y-3">
                      <button
                        ...
                        onClick={() => {
                          const amountToDeduct = payrollPaymentAmountUsd !== '' ? parseFloat(payrollPaymentAmountUsd) : maxPayment;
                          const amountHandedOver = paymentWorker.salary - amountToDeduct;
                          if (onProcessPayroll) {
                            onProcessPayroll(paymentWorker.id, amountHandedOver, amountToDeduct, 'Cash', todayRate);
                            setIsPayrollPaymentModalOpen(false);
                      ...
                      <button
                        onClick={() => {
                          ...
                          const amountHandedOver = paymentWorker.salary - amountToDeduct;
                          if (onProcessPayroll) {
                            onProcessPayroll(paymentWorker.id, amountHandedOver, amountToDeduct, 'PagoMovil', todayRate);
                            setIsPayrollPaymentModalOpen(false);
                            setPayrollPaymentAmountUsd('');
                          }
                        }}
                      ...
                      <button
                        onClick={() => {
                          ...
                          const amountHandedOver = paymentWorker.salary - amountToDeduct;
                          if (onProcessPayroll) {
                            onProcessPayroll(paymentWorker.id, amountHandedOver, amountToDeduct, 'Transfer', todayRate);
                            setIsPayrollPaymentModalOpen(false);
                            setPayrollPaymentAmountUsd('');
                          }
                        }}
